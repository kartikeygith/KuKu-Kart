...
import { getEstimatedDelivery, INDIAN_CITIES_PINCODES } from '../../utils/helpers';
...
const ProductDetail = () => {
  ...
  const [pincodeInput, setPincodeInput] = useState(pincode || '110001');
  ...
  const [reviewsList, setReviewsList] = useState([
    {
      ...
      comment: 'Unbelievable hand-finished perfection. The packaging and presentation reflect true luxury. Arrived within 24 hours via white-glove courier.',
      date: '2 days ago',
      verified: true
    },
    {
      ...
      comment: 'The drape and materials feel sublime. The sizing is exact according to the atelier chart. Truly a signature acquisition.',
      date: '1 week ago',
      verified: true
    }
  ]);
  ...
  const handleAddToCart = () => {
    ...
    setToastMessage('Added to your acquisition selection!');
  ...
  const handlePincodeCheck = (e) => {
    ...
    if (INDIAN_CITIES_PINCODES[pincodeInput]) {
      ...
      setPincodeMessage({
        success: true,
        text: `Insured delivery available by ${getEstimatedDelivery(2)} (Free Delivery)`
      });
    } else {
      setPincodeMessage({
        success: false,
  ...
  const handleAddReview = (e) => {
    ...
    const newRev = {
      ...
      location: 'New Delhi',
      rating: reviewRating,
      title: reviewTitle || 'Masterpiece Acquisition',
    ...
    <div className="pdp-page">
      ...
      <div className="pdp-main-content container flex gap-12 py-8">
        ...
        <div className="pdp-gallery-area flex-1">
          <div className="main-zoom-box relative">
            <img src={selectedImage} alt={product.title} className="main-pdp-img" />
            {product.discountPercent > 0 && (
              <span className="pdp-floating-discount">{product.discountPercent}% OFF</span>
            )}
          </div>

          {/* Thumbnails (Simulating multi-angle views) */}
        ...
        <div className="pdp-info-area flex-1 flex-col">
          <div className="brand-header flex justify-between items-center">
            ...
            <span className="text-10 text-muted">SKU: KK-{product.id.toUpperCase()}</span>
          </div>
          ...
          {/* Rating Badge */}
          <div className="pdp-rating-row flex items-center gap-3 my-4 pb-4 border-b border-border">
            <div className="flex items-center gap-1 bg-surface px-2 py-1 border border-border text-xs text-white">
              <span className="font-bold">{product.rating}</span>
              <Star size={12} fill="var(--color-accent)" color="var(--color-accent)" />
            </div>
            <span className="text-xs text-muted">| {product.reviewsCount || 128} Verified Reviews</span>
            <span className="text-xs text-success font-semibold">100% In Stock & Ready to Dispatch</span>
          </div>

          {/* Price Box */}
          <div className="pdp-price-section my-2">
            <div className="flex items-baseline gap-3">
              <span className="pdp-final-price font-bold text-accent">${product.price.toLocaleString()}</span>
              {product.originalPrice && (
                <span className="pdp-mrp-price text-muted text-sm line-through">${product.originalPrice.toLocaleString()}</span>
              )}
              {product.discountPercent > 0 && (
                <span className="pdp-discount-pill text-xs font-bold">SAVE {product.discountPercent}%</span>
              )}
            </div>
            <span className="text-10 text-muted block mt-1">Inclusive of all concierge import taxes & insured duties</span>
          </div>
            ...
            <div className="variant-block my-4">
              <div className="flex justify-between items-center mb-2">
                <span className="variant-label text-xs uppercase font-bold tracking-wider text-white">
                  SELECT SIZE / SPECIFICATION: <span className="text-accent">{selectedSize}</span>
                </span>
                <button 
          ...
          {product.colors && product.colors.length > 0 && (
            <div className="variant-block my-4">
              <span className="variant-label text-xs uppercase font-bold tracking-wider text-white block mb-2">
                FINISH & COLORWAY: <span className="text-accent">{selectedColor}</span>
              </span>
          ...
          {/* Pincode Estimator (Like Myntra / JioMart Pincode Check) */}
          <div className="pincode-estimator-card p-4 border border-border bg-surface my-4">
            ...
            <form onSubmit={handlePincodeCheck} className="flex gap-2">
              ...
              />
              <button type="submit" className="btn-secondary text-xs px-4">CHECK</button>
            </form>
            {pincodeMessage && (
          ...
          <div className="pdp-actions-row flex gap-4 my-6">
            <button onClick={handleBuyNow} className="btn-primary flex-1 text-center py-4 font-bold tracking-wider">
              PROCEED TO BUY NOW →
            </button>
            <button onClick={handleAddToCart} className="btn-secondary flex-1 text-center py-4 font-bold tracking-wider flex items-center justify-center gap-2">
          ...
          <div className="pdp-perks-grid grid-2 gap-3 pt-6 border-t border-border">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-col">
                ...
                <span className="text-10 text-muted">Direct express insured courier</span>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <ShieldCheck size={20} color="var(--color-accent)" />
              <div className="flex-col">
                <strong className="text-xs text-white">5-Year Warranty</strong>
                <span className="text-10 text-muted">Complete global coverage</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="pdp-details-section container py-12 border-t border-border">
        ...
        <div className="details-card p-8 border border-border bg-surface max-w-4xl mx-auto">
          ...
          <h4 className="spec-heading text-xs text-accent tracking-widest uppercase mb-4">TECHNICAL SPECIFICATIONS</h4>
          <div className="specs-table grid-2 gap-4 text-xs">
            ...
            <div className="spec-row p-3 border border-border flex justify-between">
              ...
              <span className="text-white font-bold">Handcrafted in Italy / Switzerland</span>
            </div>
            <div className="spec-row p-3 border border-border flex justify-between">
              <span className="text-muted">Care / Maintenance:</span>
              <span className="text-white font-bold">Specialist Concierge Service</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="reviews-section container py-12 border-t border-border">
        <div className="flex justify-between items-center mb-8">
          <div>
            <span className="text-xs text-accent tracking-widest uppercase">AUTHENTIC EXPERIENCES</span>
        ...
        <div className="reviews-grid grid-2 gap-6">
          {reviewsList.map(rev => (
            <div key={rev.id} className="review-card p-6 border border-border bg-surface flex-col">
              <div className="flex justify-between items-center mb-2">
                <div>
      ...
      {similarProducts.length > 0 && (
        <section className="similar-products container py-12 border-t border-border">
          <h2 className="section-title text-center mb-8">SIMILAR MASTERPIECES</h2>
            ...
            {similarProducts.map(p => (
              <div key={p.id} className="luxury-product-card flex-col">
                ...
                <div className="card-info p-4 flex-col flex-1 justify-between">
                  ...
                  <div className="price-box mt-3 font-bold text-accent">${p.price.toLocaleString()}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="modal-overlay flex items-center justify-center">
          <div className="modal-card p-6 bg-surface border border-accent max-w-lg w-full">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-heading tracking-wider">ATELIER SIZING MATRIX</h3>
      ...
      {showReviewModal && (
        <div className="modal-overlay flex items-center justify-center">
          <div className="modal-card p-6 bg-surface border border-accent max-w-lg w-full">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-heading tracking-wider">SUBMIT VERIFIED CLIENT REVIEW</h3>
            ...
            <form onSubmit={handleAddReview} className="flex-col gap-4">
              ...
              <div className="form-group flex-col">
                ...
                <input 
                  ...
                  placeholder="e.g. Masterpiece Acquisition"
                  required 
                />
              </div>

              <div className="form-group flex-col">
                <label className="text-xs text-accent">YOUR FEEDBACK</label>
                <textarea 
                  rows="4" 
                  value={reviewComment} 
                  onChange={(e) => setReviewComment(e.target.value)} 
                  placeholder="Describe the craftsmanship, fit, and delivery experience..."
                  required 
                />
              </div>

              <button type="submit" className="btn-primary mt-2 w-full">
                SUBMIT PRIVILEGED REVIEW
              </button>
            </form>
          </div>
        </div>
